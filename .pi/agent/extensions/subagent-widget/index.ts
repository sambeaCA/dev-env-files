...
export default function(pi: ExtensionAPI) {
    ...
    try {
      ...
      if (type === "message_update") {
        ...
        if (delta?.type === "text_delta") {
          ...
          if (state.textChunks.length > 200) state.textChunks.shift();
          updateWidgets();
        }
  ...
  pi.registerTool({
    ...
    execute: async (_toolCallId, args, _signal, _onUpdate, ctx) => {
      ...
      return {
        ...
        details: { id, task: args.task, status: "running" },
      };
    },
  });
    ...
    execute: async (_toolCallId, args, _signal, _onUpdate, ctx) => {
      ...
      if (!state) {
        return {
          ...
          details: { id: args.id, status: "error", error: "not_found" },
        };
      }
      if (state.status === "running") {
        return {
          content: [{ type: "text", text: `Error: Subagent #${args.id} is still running.` }],
          details: { id: args.id, status: "error", error: "still_running" },
        };
      }
      ...
      return {
        ...
        details: { id: args.id, turnCount: state.turnCount, status: "running" },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    execute: async (_toolCallId, args, _signal, _onUpdate, ctx) => {
      ...
      return {
        ...
        details: { id: args.id, status: "removed" },
      };
    },
  });
  ...
  pi.registerTool({
    ...
    execute: async () => {
      ...
      const summary = Array.from(agents.values()).map(s => ({
        id: s.id,
        status: s.status,
      ...
      return {
        ...
        details: { count: summary.length, agents: summary },
      };
    },
  });
  ...
  pi.registerCommand("subclear", {
    ...
    handler: async (_args, ctx) => {
      ...
      ctx.ui.notify(msg, total === 0 ? "info" : "info");
    },
  });
